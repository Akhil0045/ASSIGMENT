...
const INCOME_CATEGORIES = ['All Categories', 'Revenue', 'Salary', 'Business', 'Investments', 'Freelance', 'Loan', 'Insurance', 'Other'];
const EXPENSE_CATEGORIES = ['All Categories', 'Operations', 'Rent', 'Utilities', 'Transportation', 'Software', 'Healthcare', 'Marketing', 'Equipment', 'Other'];
...
const Daily = () => {
  ...
  const [filterEmail, setFilterEmail] = useState('');   // user filter for analyst/admin
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      if (filterEmail) params.email = filterEmail;   // analyst/admin: filter by user
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Delete this financial record? This action cannot be undone.')) {
      try {
  ...
  const handleExportCSV = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', 'Financial_Records_Export.csv');
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setDebouncedSearch(searchNotes);
      setPage(1);
    }, 400);
  ...
  useEffect(() => {
    fetchTransactions();
  }, [page, filterType, filterCategory, debouncedSearch, filterEmail]);
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 p-6 md:p-8 space-y-4">
      <div className="mb-1">
        ...
        <p className="text-slate-500 mt-1">View, filter, and manage all financial entries across income and expense categories.</p>
      </div>

      {/* User Filter — Analyst / Admin only */}
      <UserFilter
        activeEmail={filterEmail}
        onFilter={(email) => { setFilterEmail(email); setPage(1); }}
        onClear={() => { setFilterEmail(''); setPage(1); }}
      />
      {/* Top Filter Bar */}
      ...
      <div className="flex items-center gap-3 mb-6">
        {canWrite && (
          <Link to="/add-transaction" className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold shadow transition-colors flex items-center">
        ...
        <button onClick={handleExportCSV} className="px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors flex items-center group">
          <svg className="w-4 h-4 mr-2 text-emerald-600 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
          Export CSV
        </button>
      </div>
      ...
      <div className="bg-white border flex-1 border-slate-200 rounded-2xl overflow-hidden flex flex-col">
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            ...
            <tbody className="divide-y divide-slate-100">
              {transactions.length === 0 ? (
                <tr>
                  <td colSpan={user?.role === 'admin' || user?.role === 'analyst' ? "8" : "7"} className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-slate-500 font-semibold">No financial records found</p>
                      <p className="text-slate-400 text-sm">Adjust filters or add a new record to get started.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                    ...
                    <tr key={t.id} className="hover:bg-slate-50 transition-colors group">
                      ...
                      {(user?.role === 'admin' || user?.role === 'analyst') && (
                        <td className="px-6 py-4 text-left">
                          <span className="text-xs font-semibold px-2.5 py-1 text-slate-700 bg-indigo-50/50 border border-indigo-100 rounded-lg whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 text-slate-500 text-sm max-w-[200px] truncate text-left">{t.description || <span className="text-slate-300 italic">No description</span>}</td>
                      <td className="px-6 py-4 text-left">
                        <span className={`px-2 py-1 text-xs font-bold rounded-lg ${isIncome ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-600'}`}>
                      ...
                      <td className="px-6 py-4 text-slate-500 text-sm whitespace-nowrap text-left">{new Date(t.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                      <td className="px-6 py-4 text-right whitespace-nowrap">
                        {((t.userId === user?.id && user?.role === 'viewer') || user?.role === 'admin') && (
                          <div className="flex justify-end space-x-1 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                            <Link to={`/edit-transaction/${t.id}`} className="p-2 text-slate-400 hover:text-indigo-600 bg-white rounded-lg border border-transparent hover:border-indigo-100 transition-all" title="Edit record">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                            </Link>
                            <button onClick={() => handleDelete(t.id)} className="p-2 text-slate-400 hover:text-rose-600 bg-white rounded-lg border border-transparent hover:border-rose-100 transition-all" title="Delete record">
        ...
        <div className="bg-white border-t border-slate-100 p-4 flex items-center justify-between">
          <span className="text-xs font-semibold text-slate-400 tracking-wide uppercase">
            {loading ? 'Loading...' : `${total} total records — Page ${page} of ${totalPages}`}
          </span>
          {totalPages > 1 && (
            <div className="flex items-center gap-1">
              <button disabled={page === 1} onClick={() => setPage((p) => p - 1)}
                className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-600 disabled:opacity-40 text-sm font-medium hover:bg-slate-50 transition-colors">
                ← Prev
              </button>
