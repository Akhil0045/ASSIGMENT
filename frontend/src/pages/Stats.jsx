...
const getDateRange = (tf) => {
  ...
  if (tf === 'Daily') {
    ...
    return { startDate: toYMD(d), endDate: toYMD(today) };
  } else if (tf === 'Weekly') {
...
const Stats = () => {
  ...
  const fetchCharts = useCallback(async (startDate, endDate) => {
    setLoading(true);
    setError('');
    try {
      const params = {};
      ...
      console.log('Fetching charts with params:', params); // debug
      const resCharts = await api.get('/transactions/charts', { params });
      setChartData({
  ...
  // Trigger fetch whenever timeframe changes (non-custom)
  useEffect(() => {
    if (timeframe !== 'Custom') {
      const { startDate, endDate } = getDateRange(timeframe);
      fetchCharts(startDate, endDate);
    }
  }, [timeframe, fetchCharts]);
  ...
  const handleCustomQuery = () => {
    ...
    setError('');
    fetchCharts(customStart, customEnd);
  };
  ...
  return (
    <div className="flex flex-col bg-slate-50 p-6 md:p-8 gap-6 min-h-full">

      {/* Header + Pill Filter Row */}
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Statistical Breakdown</h2>
          <p className="text-slate-500 mt-1">Deep visual distribution of your categorical spending.</p>
        </div>
      ...
      {timeframe === 'Custom' && (
        <div className="flex flex-wrap items-end gap-4 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          ...
          <button
            ...
            className="h-10 px-6 bg-amber-800 hover:bg-amber-900 text-white text-sm font-bold rounded-xl transition-colors shadow"
          >
            Apply Range
          </button>
          {customStart && customEnd && (
            <p className="text-xs text-slate-400 italic self-end pb-1">
              Querying from <b>{customStart}</b> to <b>{customEnd}</b>
            </p>
          )}
        </div>
      )}
      ...
      {loading ? (
        <div className="flex flex-1 items-center justify-center py-24 bg-white rounded-2xl border border-slate-100 shadow-sm">
          <p className="text-slate-400 animate-pulse font-medium text-lg">Loading chart data...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {/* Pie Chart */}
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <h3 className="text-base font-bold text-slate-800 mb-1">Expenses by Categories</h3>
